...
function fyDialogCode(params){
	...
	params.layer.open({
		...
		scrollbar:false,
		title:[params.title,'color:#FFF;border-bottom:none;background-color:#1492ff'],
		content:'<div class="dialog_form"><div class="body">'+params.content+'</div><div class="footer"><div class="error_box" style=" margin-top:10px;"></div><ul><li><input type="button" value="发码" class="btn_ok"></li><li><input type="button" value="关闭" class="btn_cancel"></li></ul></div></div>',
		success: function(layero,index){
			params.handleSuccess(layero,index);
			if(params.height!='auto'){
				var body_height=$('.layui-layer-content',layero).height()-$('.footer',layero).outerHeight()-50;
		...
		suggest:function(params){
			...
			$(this).on('keyup',function(e){
				...
				if(e.keyCode!=38&&e.keyCode!=40&&e.keyCode!=13){
					var test=[{id:23,value:'hsb@dfdf.com'},{id:23,value:'hsb@df.com'},{id:23,value:'dddhsb@df.com'},{id:23,value:'hsdddb@df.com'}];
					list(test);
					$.ajax({type:'GET',url:params.url,data:{q:q},success:list,dataType:"json"});
				}
			});
			$(q).on('keydown',function(e){
				$('.list a',s).removeClass('selected');
				if(e.keyCode==38){//up
					if(last_run=='down'){
						tab=cur-1;
						last_run='up';
					}else if(last_run==''){
						tab=$('.list a',s).length-1;
						cur=tab;
						last_run='up';
					}
					cur=tab;
					if(tab>0){
						$('.list a:eq('+tab--+')',s).addClass('selected');
					}else{
						$('.list a:eq('+tab+')',s).addClass('selected');
						tab=$('.list a',s).length-1;
					}
					...
					$(q).val($('.list a:eq('+cur+')',s).text());
					params.callback({id:$('.list a:eq('+cur+')',s).attr('data-id'),value:$('.list a:eq('+cur+')',s).text()});
					$('.list',s).remove();
					last_run='';
					cur=0;
					tab=0;
			...
			function list(res){
				...
				$('.list',s).remove();
				$(s).append('<ul class="list"></ul>');
				var list=$('.list');
				list.css({width:($(q).outerWidth())+'px',top:($(s).outerHeight())+'px'});
				var items='';
				$.each(res,function(i,item){
					items+='<li><a href="javascript:void(0);" tabindex="'+i+'" data-id="'+item.id+'">'+item.value+'</a></li>';
				});
				list.append(items);
				$('a',list).on('click',function(){
					$(q).val($(this).text());
					params.callback({id:$(this).attr('data-id'),value:$(this).text()});
					$(list).remove();
				});
				$('a',list).hover(function(){
					tab=$('a',list).index($(this));
					cur=tab;
